...
pub fn main() void {
    ...
    const meal: Food = food_loop: for (menu) |food| {
        ...
        ingredient_loop: for (food.requires) |required, required_ingredient| {
            ...
            for (wanted_ingredients) |want_it, position| {
                if (required_ingredient == want_it) continue :ingredient_loop else if (position >= wanted_ingredients.len - 1) continue :food_loop;
            }
